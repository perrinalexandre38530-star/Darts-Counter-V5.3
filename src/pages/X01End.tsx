...
export default function X01End({ go, params }: Props) {
  ...
  const players: PlayerLite[] = React.useMemo(() => {
    ...
    const mapped = arr.map((p: any) => ({ id: p.id, name: p?.name || "—", avatarDataUrl: p?.avatarDataUrl ?? null }));
    setPlayersById(Object.fromEntries(mapped.map(p => [p.id, p])));
    return mapped;
  }, [rec]);
  ...
  React.useEffect(() => {
    ...
    const h60 = zmap(); const h100 = zmap(); const h140 = zmap(); const h180 = zmap();
    const miss = zmap(); const bust = zmap(); const dbull = zmap();
    const missPct = zmap(); const bustPct = zmap(); const dbullPct = zmap();
    const doubles = zmap(); const triples = zmap(); const bulls = zmap();
    ...
    const order = [...ids].sort((a, b) => {
      const az = remaining[a] === 0, bz = remaining[b] === 0;
      if (az && !bz) return -1;
      if (!az && bz) return 1;
  ...
  return (
    <Shell go={go} title={
      (((rec?.kind === "x01" || rec?.kind === "leg") ? "LEG" : String(rec?.kind || "Fin").toUpperCase()) + " — " + dateStr)
    } canResume={!!resumeId && !finished} resumeId={resumeId}>
...
function btnGold():React.CSSProperties{
  return {
    ...
    color:"#141417",
    fontWeight:900,            // <- la quote en trop est supprimée
