...
// Version V2 basée sur l'ancien X01Play "stats OK"
...
// - Miss / Bust / DBull live + merge dans LegResult
// - computeLegStats() + aggregateMatch() + saveMatchStats()
...
/* =========================================================
   Helper : résumé standardisé X01 (compat StatsHub)
========================================================= */
...
function readStartParams(
  ...
  const fromParams: Partial<StartParams> =
    (params?.startParams ?? {}) as Partial<StartParams>;
  ...
  const merged: StartParams = {
    playerIds:
      fromParams.playerIds ??
      fromGlobal.playerIds ??
      fromProps.playerIds ??
      [],
    start: (fromParams.start ??
      fromGlobal.start ??
      fromProps.start ??
      501) as 301 | 501 | 701 | 901,
    ...
    inMode: (fromParams.inMode ??
      ...
      "simple") as Mode,
    setsToWin:
      fromParams.setsToWin ??
      fromGlobal.setsToWin ??
      fromProps.setsToWin ??
      1,
    legsPerSet:
      fromParams.legsPerSet ??
      fromGlobal.legsPerSet ??
      fromProps.legsPerSet ??
      1,
    finishPolicy: (fromParams.finishPolicy ??
      fromGlobal.finishPolicy ??
      ("firstToZero" as FinishPolicy)) as FinishPolicy,
    officialMatch:
      fromParams.officialMatch ?? fromGlobal.officialMatch ?? false,
...
function pickWinnerId(
  res: LegacyLegResultLite | import("../lib/stats").LegStats
) {
...
function suggestCheckout(
  ...
  if (dartsLeft === 1) {
    if (doubleOut) {
      ...
      if (rest % 2 === 0 && rest / 2 >= 1 && rest / 2 <= 20) {
        return [`D${rest / 2}`];
      }
      return [];
    } else {
...
/* =========================================================
   Composant principal
========================================================= */
export default function X01PlayV2({
  profiles = [],
  playerIds = [],
  start = 501,
  outMode = "double",
  ...
  const resumeSnapshot = React.useMemo<X01Snapshot | null>(() => {
    ...
    const rec: SavedMatch | null | undefined =
      (History as any).getX01?.(resumeId) ?? History.get(resumeId);
  ...
  const [lastLegResult, setLastLegResult] =
    React.useState<LegResult | null>(null);
  ...
  function pushVisitLog(visit: Visit) {
    setVisitsLog((prev) => {
      const arr = [...(prev || [])];
      const segs = Array.isArray(visit?.darts)
        ? visit.darts.map((d) => ({
            v: Number(d?.v ?? 0),
  ...
  const defaultFinishPolicy: FinishPolicy =
    finishPref ??
    ((safeGetLocalStorage("opt_continue_policy") ??
      "firstToZero") as FinishPolicy);
  ...
  const {
    ...
    onLegEnd: async (res: LegResult) => {
      ...
      for (const pid of Object.keys(missByPlayer)) {
        (res as any).miss[pid] =
          ((res as any).miss[pid] || 0) + (missByPlayer[pid] || 0);
      }
      for (const pid of Object.keys(bustByPlayer)) {
        (res as any).bust[pid] =
          ((res as any).bust[pid] || 0) + (bustByPlayer[pid] || 0);
      }
      for (const pid of Object.keys(dbullByPlayer)) {
        (res as any).dbull[pid] =
          ((res as any).dbull[pid] || 0) + (dbullByPlayer[pid] || 0);
      }
      ...
      try {
        const legInput: LegInput = {
          startScore,
          players: ((state.players || []) as { id: string }[]).map(
            (p) => p.id
          ),
          visits: visitsLog,
          finishedAt: Date.now(),
        ...
        try {
          const playersArr = ((state.players || []) as EnginePlayer[]).map(
            (p) => ({
              id: p.id,
              name: p.name,
            })
          ...
          for (const pid of legStats.players) {
            ...
            statsParJoueur[pid] = {
              avg3:
                typeof pr.avg3 === "number"
                  ? pr.avg3
                  : 0,
              bestVisit:
                typeof pr.bestVisit === "number"
                  ? pr.bestVisit
                  : 0,
              bestCheckout:
                typeof pr.bestCheckout === "number"
                  ? pr.bestCheckout
                  : 0,
              darts:
                typeof pr.dartsThrown === "number"
                  ? pr.dartsThrown
                  : 0,
              win: (res.winnerId ?? null)
                ? res.winnerId === pid
                : !!pr.win,
              buckets:
                pr.buckets && Object.keys(pr.buckets).length
                  ? pr.buckets
                  : undefined,
            };
          }
  ...
  React.useEffect(() => {
    ...
    const playersNow = ((state.players || []) as EnginePlayer[]).map(
      (p) => ({
        id: p.id,
        name: p.name,
      })
    ...
    for (const pid of ids) {
      ...
      const visits =
        (res as any)?.visits?.[pid] || Math.ceil(dartsThrown / 3);
  ...
  function buildEngineLike(
    ...
    const idx = playersArr.findIndex(
      (p) => p.id === (currentPlayer?.id as string)
    );
  ...
  const [lastByPlayer, setLastByPlayer] =
    React.useState<Record<string, UIDart[]>>({});
  const [lastBustByPlayer, setLastBustByPlayer] =
    React.useState<Record<string, boolean>>({});
  const [dartsCount, setDartsCount] =
    React.useState<Record<string, number>>({});
  const [pointsSum, setPointsSum] =
    React.useState<Record<string, number>>({});
  const [visitsCount, setVisitsCount] =
    React.useState<Record<string, number>>({});
  const [bestVisitByPlayer, setBestVisitByPlayer] =
    React.useState<Record<string, number>>({});
  const [missByPlayer, setMissByPlayer] =
    React.useState<Record<string, number>>({});
  const [bustByPlayer, setBustByPlayer] =
    React.useState<Record<string, number>>({});
  const [dbullByPlayer, setDBullByPlayer] =
    React.useState<Record<string, number>>({});

  const [hitsByPlayer, setHitsByPlayer] = React.useState<
    Record<
      string,
      { h60: number; h100: number; h140: number; h180: number }
    >
  >({});
  ...
  function playDartSfx(d: UIDart, nextThrow: UIDart[]) {
    const visitSum = nextThrow.reduce(
      (s, x) => s + dartValue(x),
      0
    );
  ...
  const volleyTotal = currentThrow.reduce(
    (s, d) => s + dartValue(d),
    0
  );
  ...
  function validateThrow() {
    ...
    if (!willBust && needDoubleOut && after === 0) {
      willBust = !isDoubleFinish(currentThrow);
    }

    const ptsForStats = willBust ? 0 : volleyPts;

    // --- Comptage MISS / DBULL / BUST ---
    const missCount = currentThrow.reduce(
      (n, d) => n + (d.v === 0 ? 1 : 0),
      0
    );
    const dbullCount = currentThrow.reduce(
      (n, d) => n + (d.v === 25 && d.mult === 2 ? 1 : 0),
      0
    );
    ...
    if (missCount > 0) {
      setMissByPlayer((m) => ({
        ...m,
        [currentPlayer.id]: (m[currentPlayer.id] || 0) + missCount,
      }));
    }
    if (dbullCount > 0) {
      setDBullByPlayer((m) => ({
        ...m,
        [currentPlayer.id]: (m[currentPlayer.id] || 0) + dbullCount,
      }));
    }
    if (isBust) {
      setBustByPlayer((m) => ({
    ...
    setLastByPlayer((m) => ({
      ...m,
      [currentPlayer.id]: currentThrow,
    }));
    setLastBustByPlayer((m) => ({
      ...m,
      [currentPlayer.id]: !!willBust,
    }));
  ...
  if (!state.players?.length) {
    return (
      ...
      >
        <button
          onClick={() =>
            pendingFinish ? flushPendingFinish() : onExit()
          }
          style={goldBtn}
        >
          ← Quitter
  ...
  const currentAvatar =
    (currentPlayer &&
      (profileById[currentPlayer.id]
        ?.avatarDataUrl as string | null)) ?? null;

  const curDarts = currentPlayer
    ? dartsCount[currentPlayer.id] || 0
    : 0;
  const curPts = currentPlayer
    ? pointsSum[currentPlayer.id] || 0
    : 0;
  const curM3D =
    curDarts > 0
      ? ((curPts / curDarts) * 3).toFixed(2)
      : "0.00";
  ...
  React.useEffect(() => {
    ...
    if (justFinished) {
      ...
      persistOnFinish();
      try {
        const maybeLeg: RichLegStats | undefined = (lastLegResult as any)
          ?.__legStats;
        if (maybeLeg) {
    ...
    const voice =
      (safeGetLocalStorage("opt_voice") ?? "true") === "true";
    if (!justFinished || !voice || !("speechSynthesis" in window))
      return;
  ...
  React.useEffect(() => {
    ...
    if (!lastLegResult) {
      const playersArr = ((state.players || []) as {
        id: string;
        name: string;
      }[]);
  ...
  return (
    <div
      ...
      style={{
        paddingBottom:
          Math.round(KEYPAD_HEIGHT * KEYPAD_SCALE) +
          NAV_HEIGHT +
          16,
      }}
    >
      ...
      >
        <HeaderBlock
          ...
          curM3D={curM3D}
          bestVisit={
            bestVisitByPlayer[currentPlayer?.id ?? ""] ??
            0
          }
      ...
      <div
        style={{
          height:
            Math.round(KEYPAD_HEIGHT * KEYPAD_SCALE) + 8,
        }}
      />
      ...
      <EndOfLegOverlay
        ...
        playersById={React.useMemo(
          () =>
            Object.fromEntries(
              ((state.players || []) as EnginePlayer[]).map(
                (p) => {
        ...
        onSave={(res) => {
          try {
            ...
            console.warn(
              "Impossible de sauvegarder la manche:",
              e
            );
          }
          setOverlayOpen(false);
  ...
  function HeaderBlock(props: {
    ...
    const predictedAfter = Math.max(
      currentRemaining - volleyTotal,
      0
    );
            ...
            >
              <div style={miniText}>
                ...
                <div>
                  Volée :{" "}
                  <b>
                    {Math.min(currentThrow.length, 3)}/3
                  </b>
                </div>
              </div>
            </div>
        ...
        {playersOpen && (
          ...
          >
            {statePlayers.map((p) => {
              const prof = profileById[p.id];
              const avatarSrc =
                (prof?.avatarDataUrl as string | null) ??
                null;
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Le premier joueur a terminé. Tu peux{" "}
            <b>terminer maintenant</b> (classement figé)
