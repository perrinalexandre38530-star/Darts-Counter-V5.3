...
import type { TournamentPlayer, TournamentStage, TournamentMode } from "../lib/tournaments/types";
...
export default function TournamentCreate({ store, go }: Props) {
  ...
  function buildStages(): TournamentStage[] {
    if (format === "rr") {
      return [{ id: "stage-rr", type: "round_robin", groups: 1, seeding: "random", name: "Round Robin" }];
    }
    if (format === "se") {
      return [{ id: "stage-se", type: "single_elim", seeding: "random", name: "Élimination" }];
    }
    // rr + finale
    return [
      { id: "stage-rr", type: "round_robin", groups: 2, qualifiersPerGroup: 2, seeding: "random", name: "Poules" },
      { id: "stage-se", type: "single_elim", seeding: "random", name: "Finale" },
    ];
  }
  ...
  return (
    <div style={{ minHeight: "100vh", padding: 16, paddingBottom: 90, background: theme.bg, color: theme.text }}>
      ...
      <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
        <div style={{ borderRadius: 18, border: `1px solid ${theme.borderSoft}`, background: theme.card, padding: 14 }}>
          <div style={{ fontWeight: 950, marginBottom: 8 }}>Nom</div>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            style={{ width: "100%", padding: 10, borderRadius: 12, border: `1px solid ${theme.borderSoft}`, background: "rgba(0,0,0,.18)", color: theme.text }}
          />
        </div>
        ...
        <div style={{ borderRadius: 18, border: `1px solid ${theme.borderSoft}`, background: theme.card, padding: 14 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {(["x01", "cricket", "killer", "clock"] as TournamentMode[]).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                style={{
                  borderRadius: 999,
                  padding: "8px 12px",
                  border: `1px solid ${m === mode ? theme.primary : theme.borderSoft}`,
                  background: m === mode ? `${theme.primary}22` : "rgba(0,0,0,.10)",
                  color: theme.text,
                  fontWeight: 900,
                  cursor: "pointer",
                }}
          ...
          <div style={{ marginTop: 8, fontSize: 12.5, opacity: 0.8 }}>
            (La config détaillée par mode arrive juste après : X01 legs/sets, Cricket rules, Killer rules, Horloge…)
          </div>
        </div>
        ...
        <div style={{ borderRadius: 18, border: `1px solid ${theme.borderSoft}`, background: theme.card, padding: 14 }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            <label style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <input type="radio" checked={format === "rr"} onChange={() => setFormat("rr")} />
              <span>Round Robin (tous contre tous)</span>
            </label>
            <label style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <input type="radio" checked={format === "se"} onChange={() => setFormat("se")} />
              <span>Élimination directe (bracket)</span>
            </label>
            <label style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <input type="radio" checked={format === "rr_se"} onChange={() => setFormat("rr_se")} />
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: checked ? `${theme.primary}18` : "rgba(0,0,0,.12)",
                    color: theme.text,
                    cursor: "pointer",
                  }}
                >
                  ...
                  >
                    {p.avatarDataUrl ? (
                      <img src={p.avatarDataUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : null}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 950 }}>{p.name || "Joueur"}</div>
                    <div style={{ fontSize: 12, opacity: 0.75 }}>{checked ? "Sélectionné" : "Non sélectionné"}</div>
                  </div>
